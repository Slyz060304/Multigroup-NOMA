function [ q_matrix_Alg,G_count_avg,D_count ] = Alg_WEE_PowerAllocation( Weight_Total,QoS,eta,beta,theta,xi,rho,Psi,P_max ) 
global M
global N
global Channel
global P_T
...
Lag_EE_up = max(max(Weight_Total.*Channel));
Lag_EE_down = 0;
Lag_gap = 10^-10;
...
while Lag_EE_up-Lag_EE_down>Lag_gap && sum(Psi(1,:))<=P_max
    Lag_EE_mid =(Lag_EE_up+Lag_EE_down)/2;
    Power_Lag = 0;
    G_count = G_count+1;
    for m=1:M
        Dev_count = 1;
        while Dev_count<N && Dev_Total(Dev_count+1,m)<Lag_EE_mid && Dev_Total(Dev_count+1,m)>0
        ...
        Power_Lag_tmp = weight_Lag/(Lag_EE_mid*log(2))+rho_Lag/xi_Lag-1/(alpha_Lag*xi_Lag);
    ...
    q_matrix_Alg = Q_Alloc( Power_Lag_save,Index_total,Monot_total,Index_t,Thr_total,eta,beta,xi,rho );
    Rate_total_Alg = Rate_Stat( q_matrix_Alg );
    if Power_Lag>P_max
        Lag_EE_down = Lag_EE_mid;
    else
        if sum(sum(Rate_total_Alg.*Weight_Total))-Lag_EE_mid*(sum(Power_Lag_save)+P_T)>0
            Lag_EE_down = Lag_EE_mid;
        else
            Lag_EE_up = Lag_EE_mid;
        end
    end
end
...
if sum(Power_Lag_save)-P_max>P_max/1000
    Power_Lag_save = Power_Lag_save+(P_max-sum(Power_Lag_save))/M;
    if min(Power_Lag_save-Psi(1,:))<0
        Power_Lag_save = Psi(1,:)+(P_max-sum(Psi(1,:)))/M;
    end
end

Power_Lag_save = Power_Lag_save';
